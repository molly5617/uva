...
def set_title(title):
    system(
        f"title {title}" if os.name == "nt" else f"PS1='\\[\\e]0;{title}\\a\\]'; clear"
    )


def blue(text):
    return "".join(
        f"\033[38;2;{30 + i * 3};{144 + i * 3};{255}m{c}" for i, c in enumerate(text)
    )


def yellow(text):
    return "".join(
        f"\033[38;2;{255 - i * 3};{255 - i * 3};{102 + i * 2}m{c}"
...
def red(text):
    return "".join(
        f"\033[38;2;255;{100 + i * 3};{100 + i * 3}m{c}\033[0m"
        for i, c in enumerate(text)
    )
...
def uwu2(link):
    ...
    try:
        response = requests.get(
            f"https://api.toksaver.com/bypass/{link}", headers=headers
        )
...
if __name__ == "__main__":
    ...
    while True:
        ...
        set_title(
            f"Linkvertise Bypasser - Successful: {successful_links} - Failed: {failed_links}"
        )
